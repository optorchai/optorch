"""list individuals with SCIM filtering - mysql"""
...
class ListIndividualsFilteredQuery(BaseQuery):
    ...
    async def execute(
        ...
        if organization_id:
            base_where += " AND om.organization_id = %(org_id)s"
            params["org_id"] = organization_id
        
        if where_clause:
            # convert :param_N to %(param_N)s for MySQL
            mysql_where = where_clause
            for key in filter_params:
                mysql_where = mysql_where.replace(f":{key}", f"%({key})s")
            base_where += f" AND ({mysql_where})"
