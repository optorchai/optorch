...
class ChromaDBVectorStore(BaseVectorStore):
    """ChromaDB vector storage - local or persistent"""
    
    def __init__(self, embedding_provider, persist_directory: Optional[str] = None, config_manager: Optional['ConfigManager'] = None, **kwargs):
        ...
        if persist_directory is None:
            ...
            persist_directory = config_manager.secret_provider.get("CHROMADB_PERSIST_DIR")
        ...
        logger.info(f"ChromaDB initialized (persist: {persist_directory or 'memory'})")
    ...
    async def index_messages(self, messages: List[Message], ctx: MessageContext) -> None:
        ...
        logger.info(f"Indexed {len(texts)} messages (session {ctx.session_id})")
    ...
    def clear_session(self, session_id: str) -> None:
        ...
        try:
            ...
            logger.info(f"Cleared collection for session {session_id}")
