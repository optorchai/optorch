...
class MySQLStore(AbstractStore):
    ...
    async def connect(self) -> None:
        ...
        self.db = Database(self.connection_string, max_size=self.pool_size)
        await self.db.connect()
        logger.info(f"mysql connected: {self.connection_string}")
    ...
    async def prepare(self) -> None:
        """prepare mysql - set isolation level, charset, etc"""
        if self.db:
            await self.db.execute("SET NAMES utf8mb4")
            await self.db.execute("SET CHARACTER SET utf8mb4")
        logger.info("mysql prepare complete")
